...
export const GADGETS: GadgetDef[] = [
  ...
  effectGadget("mov_rdi_r13", 0x400240n, "mov rdi, r13; ret",
    "rdi = r13", "move",
    s => { s.regs.rdi = s.regs.r13; }),

  effectGadget("mov_rsp_rax", 0x400320n, "mov rsp, rax; ret",
    "rsp = rax (stack pivot — the next ret pops from the new rsp)", "pivot",
    s => { s.regs.rsp = s.regs.rax; }),
  effectGadget("mov_rsp_r13", 0x400328n, "mov rsp, r13; ret",
    "rsp = r13 (stack pivot via the r13 register)", "pivot",
    s => { s.regs.rsp = s.regs.r13; }),
...
export function popQword(s: VmState): bigint {
  const rsp = s.regs.rsp;
  const stackEnd = s.stackBaseAddr + BigInt(s.stack.length * 8);
  if (rsp >= s.stackBaseAddr && rsp < stackEnd) {
    const idx = Number(rsp - s.stackBaseAddr) / 8;
    if (!Number.isInteger(idx)) {
      s.halted = true;
      s.error = `unaligned rsp=0x${rsp.toString(16)}`;
      return 0n;
    }
    const v = s.stack[idx];
    s.regs.rsp = (rsp + 8n) & MASK64;
    return v;
  }
  if (rsp >= s.bssStart && rsp + 8n <= s.bssEnd) {
    let v = 0n;
    for (let i = 0; i < 8; i++) {
      const b = s.mem.get((rsp + BigInt(i)).toString());
      if (b === undefined) {
        s.halted = true;
        s.error = `pivoted rsp=0x${rsp.toString(16)} reads uninitialised .bss byte at 0x${(rsp + BigInt(i)).toString(16)}`;
        return 0n;
      }
      v |= BigInt(b) << BigInt(i * 8);
    }
  ...
  s.halted = true;
  s.error = `rsp=0x${rsp.toString(16)} is not a valid stack address (must be in main stack or pivoted into .bss)`;
  return 0n;
}
