...
export function judge(
  ...
  testCases?: TestCase[],
  maxBlocks?: number,
  stdinIsKidControlled?: boolean,
): JudgeResult {
  if (typeof maxBlocks === "number" && flowchart.nodes.length > maxBlocks) {
    return {
      correct: false,
      output: "",
      error: `chain has ${flowchart.nodes.length} blocks, limit is ${maxBlocks}`,
      steps: 0,
    };
  }
  ...
  if (blacklistedBytes && blacklistedBytes.length > 0) {
    ...
    if (stdinIsKidControlled && init.stdin) {
      for (let i = 0; i < init.stdin.length; i++) {
        const byte = init.stdin.charCodeAt(i) & 0xff;
        if (bad.has(byte)) {
          const ch =
            byte >= 32 && byte < 127 ? ` ('${String.fromCharCode(byte)}')` : "";
          return {
            correct: false,
            output: "",
            error: `stdin byte ${i} is forbidden: 0x${byte.toString(16).padStart(2, "0")}${ch}`,
            steps: 0,
          };
        }
      }
